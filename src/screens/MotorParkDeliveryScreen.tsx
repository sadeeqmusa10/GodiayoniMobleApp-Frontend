...
import { useCreateMyMotorParkDelivery } from "../Api/LogisticsApi";
...
export default function MotorParkDeliveryScreen() {
  const navigation = useNavigation<NavigationProp>();
  const { createMotorParkDelivery, isPending } = useCreateMyMotorParkDelivery();
  ...
  const handleSubmit = async (formData: FormData) => {
    try {
      const { deliveryId } = await createMotorParkDelivery(formData);
